...
public class MoveToAprilTagPosition extends Command {
  ...
  private PIDController areaController = new PIDController(0.03, 0, 0.0015);
  private CommandXboxController controller;
  private double MaxAngularRate = RotationsPerSecond.of(0.75).in(RadiansPerSecond);
  private double offsetOfDesired = 0;
  private double desiredXDistance = 1; // Temp???
  private double desiredArea = 1; // Temp???
  // private boolean isLeftSide;

  // (swerve, Limelight2, 0, driver, false
  public MoveToAprilTagPosition(CommandSwerveDrivetrain drivetrain, LimeLightSubsystem Limelight, int pipeline,
  ...
  public MoveToAprilTagPosition(CommandSwerveDrivetrain drivetrain, LimeLightSubsystem Limelight, int pipeline,
      CommandXboxController controller, boolean robotcentric, double offsetOfDesired, double desiredXDistance,
      double desiredArea) {
    addRequirements(drivetrain);
    m_Drivetrain = drivetrain;
    m_Limelight = Limelight;
    ...
    this.desiredXDistance = desiredXDistance;
  ...
  public void initialize() {
    ...
    areaController.setTolerance(1); // Temp value
  }
  ...
  public void execute() {
    ...
    if (m_Limelight.hasTarget()) {
      ...
      double horizontalDistance = m_Limelight.get3dXPosition();
      double currentAprilTagArea = m_Limelight.getAprilTagArea();
      double rotationSetpoint = offsetOfDesired;// + Math.toRadians(m_skew.get());
      double sideSetpoint = desiredXDistance;
      double areaSetpoint = desiredArea;
      ...
      // area (foward value)
      if (!areaController.atSetpoint()) {
        SmartDashboard.putNumber("Area setpoint", areaSetpoint);
        fowardOutput = areaController.calculate(currentAprilTagArea, areaSetpoint);
      }
      SmartDashboard.putNumber("Area Targeting error", currentAprilTagArea);
    } else {
  ...
  public boolean isFinished() {
    ...
    // <= 3;
    return thetaController.atSetpoint() && sideController.atSetpoint() && areaController.atSetpoint();
  }
}