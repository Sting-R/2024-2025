...
// Math.min(0.0,
// -new Translation2d(drivetrain.getFieldVelocity().dx,
// drivetrain.getFieldVelocity().dy)
...
// drivetrain.getYawRate().getRadians());
...
// elevHeightScalar = MathUtil.clamp(elevHeightScalar, 0.0,
// ElevatorConstants.MAX_HEIGHT.getInches());
// elevHeightScalar = ScreamMath.mapRange(elevHeightScalar, 0.0,
// ElevatorConstants.MAX_HEIGHT.getInches(), 1.0,
// 0.5);
...
// .transformBy(GeomUtil.translationToTransform(driveController.getSetpoint().position,
// 0.0))
// .getTranslation();
...
// currentPose.getTranslation().minus(targetPose.getTranslation()).getAngle())
// .transformBy(GeomUtil.translationToTransform(driveVelocity,
// 0.0)).getTranslation();
...
// if (yOverride.isPresent() && Math.abs(targetPose.minus(currentPose).getX()) <
// driveTolerance) {
...
// velocity = new Translation2d(MathUtil.clamp(velocity.getNorm(), 0.0,
// elevHeightScalar),
// velocity.getAngle());
// }

// drivetrain.setControl(drivetrain.getHelper()
// .getApplyFieldSpeeds(new ChassisSpeeds(velocity.getX(), velocity.getY(),
// headingVelocity)));
...
// Logger.log("DriveToPose/DistanceSetpoint",
// driveController.getSetpoint().position, logFrequency);
// Logger.log("DriveToPose/MeasuredHeading",
// currentPose.getRotation().getDegrees(), logFrequency);
// Logger.log("DriveToPose/SetpointHeading",
// targetPose.getRotation().getDegrees(), logFrequency);
// Logger.log("DriveToPose/Setpoint",
// new Pose2d(lastSetpointTranslation, new
// Rotation2d(headingController.getSetpoint().position)),
// logFrequency);
...
// ProfiledPIDController tempDriveController =
// DrivetrainConstants.DRIVE_ALIGNMENT_CONTROLLER;
// ProfiledPIDController tempHeadingController =
// DrivetrainConstants.HEADING_CONTROLLER_PROFILED;
